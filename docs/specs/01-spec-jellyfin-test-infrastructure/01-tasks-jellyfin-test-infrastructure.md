...
### [x] 3.0 First-Run Wizard Automation + Smoke Tests
...
- [x] 3.4 **GREEN (wizard):** Add wizard completion to the fixture. Check wizard state via `GET /Startup/Configuration` — if it returns 200, wizard needs completing. Complete via the 4-step API sequence: `POST /Startup/Configuration` (set preferred language/metadata), `POST /Startup/User` (create admin with `TEST_ADMIN_USER`/`TEST_ADMIN_PASS`), `POST /Startup/RemoteAccess` (enable remote access), `POST /Startup/Complete`. If wizard is already complete, skip silently (idempotent). Run `make test-integration` — both smoke tests should pass.
- [x] 3.5 **GREEN (idempotency):** Run `make test-integration` a second time WITHOUT running `make jellyfin-down` first. Both tests must pass. The fixture must detect the wizard is already complete and skip it. If this fails, fix the wizard detection logic.
- [x] 3.6 Ensure all helper fixtures/values the session-scoped fixture depends on are also session-scoped or plain constants (not function-scoped fixtures). Verify no `ScopeMismatch` errors.
